...
with console.status(
    "[bold cyan]Cargando modelo spaCy (md)...[/bold cyan]", spinner="dots"
):
...
def cargar_documento(ruta_archivo: str) -> BeautifulSoup:
    ...
    try:
        ...
        console.print(
            f"[bold red]Error:[/] No se encontró el archivo '{ruta_archivo}'."
        )
        sys.exit(1)
...
def extraer_textos(soup: BeautifulSoup) -> tuple[list[str], list[dict]]:
    ...
    for el in elementos:
        ...
        mapping_metadatos.extend(
            [{"titulo": titulo_actual, "parte": parte_actual}] * len(palabras)
        )
...
def generar_chunks(
    palabras_totales: list[str], mapping_metadatos: list[dict]
) -> list[dict]:
    """
...
def buscar_tfidf(chunks: list[dict], query_lema: str) -> list[dict]:
    ...
    idf = {
        token: math.log(N / (df[token] if df[token] > 0 else 1))
        for token in query_tokens
    }
...
def buscar_hibrida(
    chunks: list[dict], query_lema: str, query_original: str
) -> list[dict]:
    """
...
def agrupar_capitulos(coincidencias: list[dict]) -> tuple[dict, list[str], int]:
    ...
    titulos_caps_ord = sorted(
        agrupados_caps.keys(),
        key=lambda t: max(r["score"] for r in agrupados_caps[t]),
        reverse=True,
    )
    titulos_prelims_ord = sorted(
        agrupados_prelims.keys(),
        key=lambda t: max(r["score"] for r in agrupados_prelims[t]),
        reverse=True,
    )
...
def generar_respuesta_rag(query: str, resultados: list[dict]) -> str:
    ...
    contexto = "\n\n".join(
        [
            f"[{i + 1}] ({r['titulo_limpio']}): {r['texto_original']}"
...
def interfaz_interactiva(
    agrupados: dict,
    titulos_ordenados: list[str],
    corte_preliminares: int,
    query_original: str,
):
    """
    ...
    while True:
        ...
        tabla_capitulos = Table(
            title=f"Top {len(titulos_mostrar)} secciones más relevantes (de {len(titulos_ordenados)} en total)",
        ...
        for i, titulo in enumerate(titulos_mostrar, start=1):
            ...
            estilo_titulo = (
                "white" if i <= corte_preliminares else "italic bright_black"
            )

            tabla_capitulos.add_row(
                str(i),
                f"[{estilo_titulo}]{titulo}[/]",
                str(len(agrupados[titulo])),
                str(max_score),
            )
        ...
        opcion_str = Prompt.ask(
            "\n[bold]Selecciona la sección (o [red]'q'[/red] para el menú principal)[/bold]",
            default="q",
            show_default=False,
        )
        ...
        try:
            ...
            if 1 <= opcion <= len(titulos_mostrar):
                ...
                if len(resultados) == 1:
                    sel = resultados[0]
                    console.clear()
                    texto_resaltado = resaltar_texto(
                        sel["texto_original"], query_original
                    )
                    panel = Panel(
                        texto_resaltado,
                        title=f"[bold cyan]{sel['titulo_limpio']}[/] - Score: [green]{sel['score']}[/]",
                        border_style="cyan",
                    )
                    console.print(panel)
                ...
                while True:
                    console.clear()
                    tabla_pasajes = Table(
                        title=f"Resultados en '{titulo_sel}'",
                        header_style="bold magenta",
                    )
                    ...
                    for j, res in enumerate(resultados, start=1):
                        ...
                        resumen = " ".join(palabras[:12]) + (
                            "..." if len(palabras) > 12 else ""
                        )
                        tabla_pasajes.add_row(
                            str(j),
                            str(res["score"]),
                            resaltar_texto(resumen, query_original),
                        )

                    console.print(tabla_pasajes)

                    sub_op = Prompt.ask(
                        "\n[bold]Selecciona el pasaje ([yellow]'v'[/yellow] volver, [red]'q'[/red] menú principal)[/bold]",
                        default="v",
                        show_default=False,
                    )
...
def main():
    ...
    parser.add_argument(
        "archivo", nargs="?", default=ruta_defecto, help="Ruta al archivo HTML"
    )
    parser.add_argument("-q", "--query", help="Texto a buscar.", default=None)
    parser.add_argument(
        "-m",
        "--modo",
        choices=["clasica", "semantica", "rag"],
        default=None,
    ...
    with console.status(
        "[bold cyan]Cargando documento y generando chunks...[/bold cyan]",
        spinner="dots",
    ):
    ...
    while True:
        ...
        # Procesamiento
        with console.status(
            "[bold cyan]Procesando consulta y ejecutando búsqueda...[/bold cyan]",
            spinner="dots",
        ):
            ...
            if modo_actual == "clasica":
                ...
                console.print(
                    f"[bold yellow]Advertencia:[/] El modo '{modo_actual}' aún no está implementado."
                )
                modo_actual = None
                query_actual = None
                Prompt.ask("\n[bold]Presiona Enter para continuar[/bold]")
        ...
        if not coincidencias:
            console.print(
                "[bold yellow]No se encontraron resultados para tu búsqueda.[/bold yellow]"
            )
            modo_actual = None
            query_actual = None
        ...
        agrupados, titulos_ordenados, corte_preliminares = agrupar_capitulos(
            coincidencias
        )

        if modo_actual == "rag":
            with console.status(
                "[bold magenta]Generando respuesta con Ollama (RAG)...[/bold magenta]",
                spinner="bouncingBar",
            ):
            ...
            console.print(
                Panel(
                    respuesta,
                    title="[bold magenta]Respuesta RAG[/bold magenta]",
        ...
        interfaz_interactiva(
            agrupados, titulos_ordenados, corte_preliminares, query_actual
        )
